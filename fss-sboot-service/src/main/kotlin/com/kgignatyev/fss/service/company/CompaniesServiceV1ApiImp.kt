...
@RestController
class CompaniesServiceV1ApiImp(val companiesRepo: CompaniesRepo, @Qualifier("mvcConversionService") val conv:ConversionService): CompaniesServiceV1Api {
    

    override fun getAllCompanies( accountId:String): ResponseEntity<List<V1Company>> {
       return ok( companiesRepo.findAll().map { conv.convert(it, V1Company::class.java)!! })
    }

    override fun createCompany( accountId: String, v1Company: V1Company?): ResponseEntity<V1Company> {
        val company = companiesRepo.save(conv.convert(v1Company, Company::class.java)!!)
